...
# Target: 高性能モデル (Qwen 2.5 32B)
# ※24GBメモリで動作する最強クラスのモデルです
TARGET_MODEL_NAME = os.environ.get("OLLAMA_MODEL_TARGET", "qwen2.5:32b")

# テストするタスク数（動作確認用に最初は 3〜5 程度に設定推奨）
NUM_TEST_TASKS = 50
...
def build_agent_models() -> Dict[str, object]:
    ...
    return {
        ...
        "target": ChatOllama(
            ...
            temperature=0,
            # Qwen 32Bはメモリを大きく食うため、ロード時間を短縮するために維持設定
            keep_alive="1h",
            num_ctx=4096, 
        ),
    }
...
# 2. 判定用モデル (Judge)
# ------------------------------------------------------------------
# Judgeは外部のGPT-4oを使うことで、PCのリソースをエージェントに集中させます
...
def run_evaluation() -> Dict[FrozenSet[str], float]:
    print("🤖 ローカルQwen(32B)による評価を開始します．")
    print(f"   Base Model  : {BASE_MODEL_NAME} (Ollama)")
    print(f"   Target Model: {TARGET_MODEL_NAME} (Ollama)")
...
if __name__ == "__main__":
    ...
    print(f"Targetモデル: {TARGET_MODEL_NAME} (Local Qwen)")
